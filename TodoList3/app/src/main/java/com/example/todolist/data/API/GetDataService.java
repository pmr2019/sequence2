...
public interface GetDataService {
    ...
    Call<ResponseItems> getItems(@Path("idTodoList") long idList, @Query("hash") String hash);
    ...
    Call<ResponseItem> postItem(@Path("idTodoList") long idListe, @Query("hash") String hash, @Query("label") String label);

    // mise à jour de : l'état d'un item
    // récupération de : version, succès, status, et l'item modifié
    @PUT("lists/{idTodoList}/items/{idItem}")
    Call<ResponseItem> putCheck(@Path("idTodoList") long idListe, @Path("idItem") long idItem, @Query("hash") String hash, @Query("check") int check);

    // suppression de : une to-do liste
    // récupération de : version, succès, status
    @DELETE("lists/{idTodoList}")
    Call<ResponseBasic> deleteTodoList(@Path("idTodoList") long idListe, @Query("hash") String hash);

    // suppression de : un item
    // récupération de : version, succès, status
    @DELETE("lists/{idTodoList}/items/{idItem}")
    Call<ResponseBasic> deleteItem(@Path("idTodoList") long idListe, @Path("idItem") long idItem, @Query("hash") String hash);
